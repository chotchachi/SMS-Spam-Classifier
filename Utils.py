...
def loadStopWordCorpus(filename):
    list_content = []
    f = codecs.open(filename, 'r', 'utf-8')
    for line in f:
        line = line.strip().split('\t')
...
def removeStopWordFromText(message, stopwords):
    output = ''
    words = message.split(' ')
    for word in words:
...
def getCountSpamOrNotSpam(labels):
    spam = 0
    non_spam = 0
    for l in labels:
        if l == 1:
...
# define bayes matrix
def configureBayesMatrix(bag_words, vectors, labels, spam, non_spam, bayes_matrix):
...
def handleMessage(message, bag_words):
    input = removeSpecialCharacter(message)
